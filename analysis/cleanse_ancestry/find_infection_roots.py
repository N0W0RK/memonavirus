...
roots_file = open("infection_roots.txt", "w")
...
for file in all_files:
    ...
    for index, row in df.iterrows():
        ...
        if row.infector_name in infected:

            infector_found = True
            infected.append(row.infectee_name)

        else:

            infected.append(row.infector_name)
            infected.append(row.infectee_name)

            roots_file.write(row.infector_name + "\n")
